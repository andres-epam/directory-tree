...
const myFormat = printf(info =>
  info.hasOwnProperty('data')
    ? `${colorize().colorize(info.level, `[LOG] -> ${info.timestamp} ${info.label} ${info.level.toUpperCase()}:`)} ${info.message} ${
        info.data
      } ${colorize().colorize(info.level, '<---------- [END LOG]')}`
    : `${colorize().colorize(info.level, `[LOG] -> ${info.timestamp} ${info.label} ${info.level.toUpperCase()}:`)} ${
        info.message
      } ${colorize().colorize(info.level, '<---------- [END LOG]')}`
);
...
exports.logger = createLogger({
  format: combine(errors({ stack: true }), addMetadata(), timestamp({ format: timezone }), label({ label: '---------->' }), myFormat),
